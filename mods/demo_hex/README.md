...
> **一个刻意的示范差异**：本示例的界面会把"看起来能点"的格子也给出去，非法点击由**规则层**
> 兜底拒绝（外壳会提示"行动力不足"之类）。`demo_river` 走的是另一条路：界面只画"规则允许的"
> 点击。两种都能自圆其说——本示例留成"规则层兜底"的样板，别当成界面写错了。
...
├── assets/           资源（音频 / 图片）；**本示例未提供**（见《需要的资源.md》），缺失时只记录不报错
